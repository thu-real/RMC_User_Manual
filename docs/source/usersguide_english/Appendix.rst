...
The RMC program package includes input files for typical test cases (see :numref:`input_files_table_eng`) and output results.
All test cases have been successfully run on the "Explorer 100" petascale cluster system at Tsinghua University (Red Hat 
Enterprise Linux 5.6), with some cases utilizing large-scale parallel computing.

.. table:: List of Input Files
  :name: input_files_table_eng

  +-------------------------------------------------+-------------------------------------------------------------+
  | Input file                                      | Description                                                 |
  +=================================================+=============================================================+
  | 3_1_PWR_assembly                                | Example of repeated geometry, PWR assembly                  |
  +-------------------------------------------------+-------------------------------------------------------------+
  | 3_2_PWR_core                                    | Example of repeated geometry, PWR core                      |
  +-------------------------------------------------+-------------------------------------------------------------+
  | 3_3_MFR_assembly                                | Example of repeated geometry, hexagonal assembly            |
  +-------------------------------------------------+-------------------------------------------------------------+
  | 3_4_MFR_core                                    | Example of repeated geometry, hexagonal core                |
  +-------------------------------------------------+-------------------------------------------------------------+
  | 6_1_Tally_PWR_pin                               | Tally example, PWR fuel pin with axial segmentation         |
  +-------------------------------------------------+-------------------------------------------------------------+
  | 6_2_Tally_Hoogenboom_core                       | Tally example, full-core Monte Carlo benchmark              |
  +-------------------------------------------------+-------------------------------------------------------------+
  | 7_1_FSC_slab                                    | Source convergence acceleration example, OECD source        | 
  |                                                 | convergence benchmark                                       |
  +-------------------------------------------------+-------------------------------------------------------------+
  | 7_2_FSC_Hoogenboom_core                         | Source convergence acceleration example, full-core Monte    |
  |                                                 | Carlo benchmark                                             |
  +-------------------------------------------------+-------------------------------------------------------------+
  | 8_1_Burn_PWR_pin                                | Burnup example, PWR fuel pin                                |
  +-------------------------------------------------+-------------------------------------------------------------+
  | 8_2_Burn_PWR_assembly                           | Burnup example, PWR assembly                                |
  +-------------------------------------------------+-------------------------------------------------------------+


Continuous Energy ACE Database
--------------------------------

The RMC program package includes a continuous energy ACE database at 300K based on ENDF7.0 (with the .30c extension), 
comprising nearly 400 nuclides (see :numref:`acedata_table_eng`). Additionally, the package contains some thermal scattering
data libraries and a database of natural elements (see :numref:`natiso_table_eng`). For more details, please refer to the xdir file.

.. table:: List of Continuous Energy ACE Database
  :name: acedata_table_eng
...
.. table:: List of Natural Elements Database
  :name: natiso_table_eng
...
The release package includes a burnup database suitable for Pressurized Water Reactors (PWRs), containing single-group cross sections 
for 1487 nuclides and fission yields for 30 nuclides.

Functionality Compatibility Matrix
------------------------------------

.. table:: Functionality Compatibility Matrix
  :name: function_compatibility_eng

  +---------------------+-------------------+---------+---------+
  | Function            |   Sub-function    | MPI     | OpenMP  |
  +=====================+===================+=========+=========+
  | Criticality         |  ——               | √       | √       |
  +---------------------+-------------------+---------+---------+
  | Burnup Calculation  | Normal Burnup     | √       | √       |
  +---------------------+-------------------+---------+---------+
  |                     | Burnup Zone Merge | √       | Future  |
  +---------------------+-------------------+---------+---------+
  | Tally               | Cell Tally        | √       | √       |
  +---------------------+-------------------+---------+---------+
  |                     | Mesh Tally        | √       | √       |
  ...
  +---------------------+-------------------+---------+---------+
  | Source Convergence  | Shannon Entropy   | √       | Future  |
  +---------------------+-------------------+---------+---------+
  |                     | Source            | √       | Future  |
  |                     | Acceleration      |         |         |
  +---------------------+-------------------+---------+---------+
  | Output and Plotting | Output Results    | √       | √       |
  +---------------------+-------------------+---------+---------+
  |                     | Plotting          | √       | √       |
  +---------------------+-------------------+---------+---------+
  | Fixed Source        | ——                | √       | ×       |
  ...
  | Dynamics Calculation|                   |         |         |
  +---------------------+-------------------+---------+---------+
  | Criticality Search  | Material Search   | √       | √       |
  +---------------------+-------------------+---------+---------+
  |                     | Geometry Search   | √       | √       |
  +---------------------+-------------------+---------+---------+
  | Iterated Fission    | ——                | √       | √       |
