...
纯中子输运计算，如果输运过程中中子能量低于1E-11MeV就被杀死。
...
.. code-block:: none
    ...
    Physics
    ParticleMode < particle type >
    Photon PPRODUCEE=<params> TTB=<params> ADTTB=<params> ANNIHILATION=<params> COHERENT=<params> PHOTONNUCLEUS=<params> DOPPLER=<params> UPERERG=<params> ERGCUTGMA=<params> ELECMULTITIMES=<params>
...
-  **TTB**\ 指定光子输运过程中是否采用不考虑轫致辐射光子角度分布的简化TTB模型处理电子，0表示不采用简化TTB模型、1表示采用简化TTB模型，默认等于0，即默认不开启；

-  **ADTTB**\ 指定光子输运过程中是否采用考虑轫致辐射光子角度分布的ADTTB模型处理电子，0表示不采用ADTTB模型、1表示采用ADTTB模型，默认等于0，即默认不开启；(注：TTB和ADTTB选项不能同时打开)
...
纯光子输运计算，光子输运过程不产生电子，也不采用简化TTB模型处理电子，处理汤普逊散射和光核反应，不进行多普勒展宽的操作，能量上限为100MeV，能量下限是1.0E-3，光子输运产生电子的倍增因子为1.0
...
-  **BREMSANGLE**\ 是否详细抽样次级轫致辐射光子的方向。默认为1，详细抽样。设置为0，则简单抽样。

-  **BREMSPHOTONMULTITIMES**\ 一次轫致辐射过程中，产生的次级轫致辐射光子的数量。应为大于等于0的实数，默认值1。次级轫致辐射光子的权重会相应除以BREMSPHOTONMULTITIMES。大于1时可减少次级轫致辐射光子的方差，小于1时可减少次级轫致辐射光子的模拟时间。

-  **BREMSEACHSUBTEP**\ 是否在每一个射程子步强制产生次级轫致辐射光子。可设置为0或者1。默认值为0。若设置为1，则要求BREMSPHOTONMULTITIMES=1，同时次级轫致辐射光子的权重会做等期望调整。

-  **BREMSERGLOSSMETHOD**\ 控制电子因轫致辐射导致的能量损失的计算方法，可设置为0,或者1。默认值1。1代表使用第一个轫致辐射光子的能量计算电子能量损失，0代表使用所有轫致辐射光子能量的平均值计算电子能量损失。
...
光电子耦合输运计算，电子能量上限100Mev，下限0.001Mev，电子输运过程产生光子，阻止本领波动，产生轫致辐射电子，详细抽样次级轫致辐射光子的方向，
不在每一个射程子步强制产生次级轫致辐射光子，使用第一个轫致辐射光子的能量计算电子能量损失，一次X射线荧光或者俄歇电子产生过程中产生1个次级X射线荧光或者俄歇电子，不产生击出电子
