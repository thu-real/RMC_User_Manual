...
#.  由于随机数设置等原因，外源子程序和结果和使用EXTERNALSOURCE选项的结果不一定严格相等，在编写正确的情况下，二者在统计学意义上应该是一致的；如果想要保证二者的严格一致，必须要参照源码中的源分布抽样流程来编写外源子程序，同时注意随机数调用过程的严格一致；
...
下面将演示如何一步步构建Python外源描述，使其与下面的源描述卡等效：
...
上述源描述中包含两个源分布，可通过随机数抽样确定每个中子从哪个源发出：
...
源2是位于栅元30内的体积源，且在x、y、z方向上满足特定分布，它的出射角度分布同样需要满足特定要求，加上源2对应的分布之后，
得到的完整的python外源子程序如下：
