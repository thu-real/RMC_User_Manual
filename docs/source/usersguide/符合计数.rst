...
.. code-block:: none
     ...
     celltally <id> particle=<1> cell=<cell_vector_group> cap=<capture_num> <nuclide_IDs> gate=<pre_delay_time gate_width> capmaxnum=<max_number> capmaxmoment=<max_number> normalization=<normal_style>
                    rossialpha = <1> accumrossialpha = <1> timebins = <number> timeinterval = <number> totaltime = <number>
...
-  **zaid**\ 新的多重性核素ID。
-  **method**\ 高斯抽样算法: 0代表使用sine/cosine抽样方法; 1代表使用Lestone方法（匹配阶数）; 3代表使用Ensslin/Santi/Beddingfield/Mayo方法（1998-2004）;
   5代表使用FREYA程序进行中子诱发、自发裂变；6代表使用LLNL裂变库进行中子诱发、自发裂变和光致裂变；7代表使用CGMF程序进行中子诱发、自发裂变；默认为3。默认为3。
...
-  **rossialpha**\ 是否输出Rossi-alpha值，0为不输出，1为输出，默认为0。
-  **accumrossialpha**\ 是否输出累积Rossi-alpha值(Feynman-alpha)，0为不输出，1为输出，默认为0。
-  **timebins**\ Rossi-alpha值的时间分bin数量，默认为1e-5s。
-  **timeinterval**\ 累积Rossi-alpha值的时间窗口宽度，单位为s，默认为0.001s。
-  **totaltime**\ Rossi-alpha与累积Rossi-alpha的总时间，单位为s，默认为0.2s。
